...
/**
* FUNCTION: lmidmean( out, mat[, sorted[, dim] ] )
*	Computes the lower midmean along a matrix dimension.
*
...
* @param {Number} [dim=2] - matrix dimension along which to compute the lower midmean. If `dim=1`, compute along matrix rows. If `dim=2`, compute along matrix columns.
* @returns {Matrix|Null} lower midmean or null
*/
function lmidmean( out, mat, sorted, dim ) {
	var values,
		delta,
	...
	if ( N%8 === 0 ) {
		...
		low = Math.ceil( N*0.125 );
		high = Math.floor( N*0.375 ) - 1;
	}
