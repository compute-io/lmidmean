...
*	Computes the interquartile mean of the values below the median in a numeric array (upper midmean).
*
...
function lmidmean( arr, sorted ) {
	...
	if ( arr.length < 6 ) {
		throw new TypeError( 'lmidmean()::invalid input argument. Lmidmean not applicable.' );
	}
	if ( !sorted ) {
		arr = arr.slice();
		arr.sort( ascending );
